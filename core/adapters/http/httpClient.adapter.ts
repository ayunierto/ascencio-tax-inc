...
export class HttpClientAdapter implements HttpAdapter {
  ...
  constructor() {
    ...
    if (!apiUrl) {
      throw new Error(
        'HttpClientAdapter: EXPO_PUBLIC_API_URL environment variable is not set.'
      );
    }
    if (!apiUrl.startsWith('http')) {
      throw new Error(
        'HttpClientAdapter: EXPO_PUBLIC_API_URL environment must start with the http or https protocol.'
      );
    }
    if (!apiUrl.endsWith('/')) {
      apiUrl += '/';
  ...
  // Create the token on each request to keep the user logged.
  ...
  private async request<T>(
    ...
    try {
      ...
      console.warn('HttpClientAdapter: Request successful:', {
        method,
        url: url.toString(),
      ...
      if (!response.ok) {
        ...
        throw new HttpError(errorMessage, response.status, responseData?.error);
      }
